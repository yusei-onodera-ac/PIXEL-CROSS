...
2026-08-04 [Dev] 社長より「ヘブライ語はいいや」と指示があり、RTL対応の手間を理由にヘブライ語対応を撤回。SupportedLanguageからHebrewを削除し、READMEも修正。
2026-08-04 [Dev] 社長がUnity Hubでプロジェクトを開こうとしたところ「見つからないエディターバージョン(2022.3.50f1)」と表示。社長の開発機には6000.5.6f1がインストール済みとのことで、ProjectSettings/ProjectVersion.txtを6000.5.6f1に更新、READMEの技術スタック欄も合わせて修正。
2026-08-04 [Dev] 社長がUnity Editorを起動したところ「プロジェクトはコンパイルエラーを含んでいます」と表示（Burstの警告とは別件）。社長がPC作業を続けられない状況とのことで、代わりにUnity Editor自身が生成したLogs/Editor.logを直接読んで原因を特定。自作スクリプトではなく、Packages/manifest.jsonに指定していたcom.unity.2d.animation(9.1.3)/com.unity.inputsystem(1.7.0)/com.unity.localization(1.4.5)がUnity 2022.3時代のバージョンのままで、Unity 6000.5では`Object.GetInstanceID()`等の廃止APIがエラー扱いになるためと判明。Web検索でUnity 6対応バージョン(12.0.3 / 1.19.0 / 1.5.9)を確認し更新。あわせて、社長不在中に練習・戦術・試合・スカウト・ガチャ・ショップ・インベントリ・ロースターの各ゲームプレイ画面コントローラー（ActionRowViewによる共通リスト表示部品含む）と、未接続だったScoutSystemのGameManager統合(TryScoutRecruit)を実装。READMEに各画面の組み方とコンパイルエラー時のトラブルシュート手順を追記。
